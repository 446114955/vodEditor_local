...
//     xml = MakeHttpRequest();
// }
var xml = (window.ActiveXObject) ? new window.ActiveXObject("microsoft.xmlhttp") : new window.XMLHttpRequest();
// 创建完整Ajax程序包
function ajax( options ){
    ...
    // 建立与服务器的链接
    if(options.type=="GET"){
        if(options.headers!=""&&options.headers!=null&&options.headers!=undefined){
            xml.setRequestHeader("x-access-token",options.headers,'Content-Type',"application/json; charset=UTF-8");
        }
        xml.send(null);
    }else if(options.headers=="json"){
        xml.setRequestHeader('Content-Type',"application/json; charset=UTF-8");
        xml.send(options.post);
    }else if(options.headers!=""&&options.headers!=null&&options.headers!=undefined){
        xml.setRequestHeader("x-access-token",options.headers,'Content-Type',"application/json; charset=UTF-8");
        xml.send(options.post);
    } else {
        xml.setRequestHeader('Content-Type',"application/json; charset=UTF-8");
